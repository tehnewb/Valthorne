...
public class AlphaOccluder2D extends Occluder2D {
    ...
    public AlphaOccluder2D(AlphaShadowShape2D shape) {
        setShape(shape);
        setTransform(0, 0, 1, 1, 0, 0, 0);
    }

    public final AlphaShadowShape2D getShape() {
        return shape;
    }
    ...
    public AlphaOccluder2D setTransform(float x, float y, float width, float height, float degrees, float originX, float originY) {
        ...
        PointLight2D.finite(width);
        PointLight2D.finite(height);
        PointLight2D.finite(degrees);
        PointLight2D.finite(originX);
        PointLight2D.finite(originY);
        float sin = (float) Math.sin(Math.toRadians(-degrees)), cos = (float) Math.cos(Math.toRadians(-degrees));
        setBasis(x + originX - originX * cos + originY * sin, y + originY - originX * sin - originY * cos, width * cos, width * sin, -height * sin, height * cos);
        return this;
    }
