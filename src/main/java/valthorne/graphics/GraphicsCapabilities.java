 ...
 */
public record GraphicsCapabilities(String vendor, String renderer, String version,
                                   boolean raster, boolean compute, boolean pathTracing,
                                   boolean filament) {
    ...
    public void requireRaster() {
        if (!raster) throw new UnsupportedOperationException(
                "Valthorne's desktop renderer requires OpenGL 3.3 / GLSL 330. Detected: " + version
                        + ". OpenGL ES requires a separate backend and is not supported yet.");
    }
}
