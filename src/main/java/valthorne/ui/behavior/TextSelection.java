...
public final class TextSelection {
    ...
    public void wordStep(boolean right, boolean extend) {
        ...
        if (right) {
            ...
            while (offset > 0 && Character.isWhitespace(text.codePointBefore(offset)))
                offset = text.offsetByCodePoints(offset, -1);
            if (offset > 0) {
                int kind = category(text.codePointBefore(offset));
                while (offset > 0 && category(text.codePointBefore(offset)) == kind)
                    offset = text.offsetByCodePoints(offset, -1);
            }
        }
        move(offset, extend);
    ...
    private static int category(int point) {
        return Character.isLetterOrDigit(point) || point == '_' || Character.getType(point) == Character.NON_SPACING_MARK ? 1 : Character.isWhitespace(point) ? 0 : 2;
    }

    public void move(int offset, boolean extend) {
        caret = Math.max(0, Math.min(text.length(), offset));
        if (caret > 0 && caret < text.length() && Character.isLowSurrogate(text.charAt(caret)) && Character.isHighSurrogate(text.charAt(caret - 1)))
            caret--;
