...
public final class Keyboard {
    private Keyboard() {}
    private static final boolean[] downKeys=new boolean[349];
    ...
    static void init(){java.util.Arrays.fill(downKeys,false);install((key,mods,down)->JGL.runTask(()->{if(key>=0&&key<downKeys.length)downKeys[key]=down;JGL.publish(down?new KeyPressEvent(key,mods):new KeyReleaseEvent(key,mods));}));installText(text->JGL.postEvent(new valthorne.event.events.TextInputEvent(text)));}
    ...
    public static boolean isKeyDown(int key){return key>=0&&key<downKeys.length&&downKeys[key];}
    @JSBody(script="return !!valthorneHost.platform.capsLock;") public static native boolean isCapsLockOn();
    public static boolean isShiftDown(){return isKeyDown(LEFT_SHIFT)||isKeyDown(RIGHT_SHIFT);}
    public static boolean isCtrlDown(){return isKeyDown(LEFT_CONTROL)||isKeyDown(RIGHT_CONTROL);}
    public static boolean isAltDown(){return isKeyDown(LEFT_ALT)||isKeyDown(RIGHT_ALT);}
    public static boolean isSuperDown(){return isKeyDown(LEFT_SUPER)||isKeyDown(RIGHT_SUPER);}
    public static void addKeyListener(KeyListener listener){JGL.subscribe(EventTypes.KEY_PRESS,listener);JGL.subscribe(EventTypes.KEY_RELEASE,listener);}
    public static void removeKeyListener(KeyListener listener){JGL.unsubscribe(EventTypes.KEY_PRESS,listener);JGL.unsubscribe(EventTypes.KEY_RELEASE,listener);}
