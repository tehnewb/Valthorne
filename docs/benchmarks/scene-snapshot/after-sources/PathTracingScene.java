...
final class PathTracingScene {
    ...
    private int instanceCount, transformCount; // Active prefixes during recollection; inactive references are removed afterward.
    ...
    private void collect(Scene3D scene) {
        ...
        instanceCount = transformCount = 0;
        ...
        if (instanceCount < instances.size()) instances.subList(instanceCount, instances.size()).clear();
        if (transformCount < transforms.size()) transforms.subList(transformCount, transforms.size()).clear();
    }
    ...
    private void visit(SceneNode3D n) {
        ...
        add(n.getModel(), n.getMaterial(), n.getWorldTransform(nextTransform()));
