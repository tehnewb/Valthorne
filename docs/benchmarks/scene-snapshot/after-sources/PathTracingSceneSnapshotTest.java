...
class PathTracingSceneSnapshotTest {
    @Test
    void releasingPackedUploadDataPreservesCapturedGeometry() {
        var scene = new Scene3D();
        scene.add(new ModelInstance3D().setModel(ModelBuilder3D.box(1, 2, 3))
                .setMaterial(material()));
        var snapshot = new PathTracingScene(scene);
        snapshot.build();
        float[] triangles = snapshot.triangleData();
        float[] nodes = snapshot.nodeData();
        float[] emitters = snapshot.emitterData();
        long signature = snapshot.signature();
        snapshot.releaseBuildData();
        assertTrue(snapshot.triangles.isEmpty());
        assertTrue(snapshot.nodes.isEmpty());
        assertTrue(snapshot.emitters.isEmpty());
        assertTrue(snapshot.textures.isEmpty());
        assertEquals(signature, snapshot.signature());
        snapshot.build();
        assertArrayEquals(triangles, snapshot.triangleData());
        assertArrayEquals(nodes, snapshot.nodeData());
        assertArrayEquals(emitters, snapshot.emitterData());
    }
