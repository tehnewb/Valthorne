...
public final class SpriteOccluder2D extends AlphaOccluder2D {
    ...
    public SpriteOccluder2D(Sprite sprite) {
        this(sprite, 127, 32);
    }
    ...
    public SpriteOccluder2D(Sprite sprite, int alphaCutoff, int cachedFrames) {
        ...
        if (alphaCutoff < 0 || alphaCutoff > 255 || cachedFrames < 1)
            throw new IllegalArgumentException("Invalid alpha cutoff/cache capacity");
        cutoff = alphaCutoff;
        capacity = cachedFrames;
        frames = new Frame[capacity];
        shapes = new AlphaShadowShape2D[capacity];
        update();
    }
    ...
    public long getExtractionCount() {
        return extractions;
    }
    ...
    public void update() {
        ...
        if (current == null || current.data != data || current.u != u || current.v != v || current.u2 != u2 || current.v2 != v2 || current.cutoff != threshold) {
            ...
            for (int i = 0; i < frameCount; i++) {
                Frame key = frames[i];
                if (key.data == data && key.u == u && key.v == v && key.u2 == u2 && key.v2 == v2 && key.cutoff == threshold) {
                    slot = i;
                    break;
                }
            }
            if (slot < 0) {
                ...
                AlphaShadowShape2D shape = extract(key);
                extractions++;
                slot = nextSlot;
                nextSlot = (nextSlot + 1) % capacity;
                frames[slot] = key;
                shapes[slot] = shape;
                frameCount = Math.min(capacity, frameCount + 1);
            }
            setShape(shapes[slot]);
            current = frames[slot];
        }
    ...
    private static AlphaShadowShape2D extract(Frame frame) {
        ...
        if (!Float.isFinite(frame.u + frame.v + frame.u2 + frame.v2))
            throw new IllegalArgumentException("Invalid sprite UVs");
        ...
        if (w > data.width() || h > data.height())
            throw new IllegalArgumentException("Repeated UVs are not supported by sprite shadows");
        boolean[] mask = new boolean[Math.multiplyExact(w, h)];
        var pixels = data.buffer();
        int base = pixels.position();
        if ((long) data.width() * data.height() * 4 > pixels.remaining())
            throw new IllegalArgumentException("Missing retained RGBA pixels");
        for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++) {
