...
public final class PathTracer3D implements AutoCloseable {
     ...
     */
    @Override public void close(){if(closed)return;closed=true;sceneCollector.clear();glDeleteProgram(compute);glDeleteProgram(present);glDeleteProgram(denoise);glDeleteVertexArrays(vao);glDeleteTextures(specularRadiance);glDeleteTextures(historySpecular);glDeleteTextures(materialGuide);glDeleteTextures(filteredSpecular);glDeleteTextures(accumulation);glDeleteTextures(historyRadiance);glDeleteTextures(historyGuide);glDeleteTextures(atlas);glDeleteTextures(guide);glDeleteTextures(filtered);for(int b:buffers)glDeleteBuffers(b);}
    /**
