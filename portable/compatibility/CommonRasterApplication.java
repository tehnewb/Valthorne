...
public final class CommonRasterApplication implements Application {
    ...
    private SceneRenderer3D renderer;
    private ShadowMap3D shadow;
    private Texture texture;
    private int frames, checks, count;

    static void main(String[] args) {
        try {
            JGL.init(new CommonRasterApplication(), JGLConfiguration.defaults().title("Shared raster rendering").size(640, 480).visible(false));
            System.out.println("COMMON_RASTER_RETURNED");
    ...
    public void init() {
        ...
        check(Window.getGraphicsCapabilities().raster(), "Raster capability");
        renderer = new SceneRenderer3D(SceneRenderer3D.Backend.RASTER);
        check(renderer.getBackend() == SceneRenderer3D.Backend.RASTER, "Explicit raster backend");
        camera.setPosition(8, -12, 9);
        camera.lookAt(0, 0, 0, 0, 0, 1);
        camera.setClipPlanes(.1f, 100);
        camera.rebuild(640, 480);
        ...
        scene.add(new BillboardSprite3D().setPosition(0, -3, 2).setSize(2, 2).setTextureRegion(new TextureRegion(texture)));
        scene.addLight(new PointLight3D().setPosition(0, -4, 6).setIntensity(8).setRange(25));
        shadow = new ShadowMap3D(256);
        shadow.getCamera().setPosition(0, -8, 10);
        shadow.getCamera().lookAt(0, 0, 0, 0, 0, 1);
        shadow.getCamera().setWorldHeight(20);
        shadow.getCamera().setClipPlanes(.1f, 50);
        check(shadow.renderIfChanged(scene, 1), "Initial shadow pass");
        check(!shadow.renderIfChanged(scene, 1) && shadow.getRenderCount() == 1, "Shadow cache");
    ...
    public void update(float dt) {
        if (++frames == 120) {
            shadow.setStrength(0);
            System.out.println("RASTER_SHADOW_DISABLED");
        }
    ...
    public void render() {
        ...
        Window.clear3D(Color.NAVY);
        renderer.render(scene, camera);
        if (frames == 2) check(renderer.rasterState().getLighting().getUploadCount() > 0, "Tiled lighting upload");
    ...
    public void dispose() {
        ...
        if (count > 0) {
            ...
            System.out.println("RASTER_BENCHMARK samples=" + count + " meanSubmitMs=" + (total / 1e6 / count) + " p95SubmitMs=" + (samples[76] / 1e6));
        }
        System.out.println("COMMON_RASTER_VALIDATED checks=" + checks);
    }
}
