...
public final class GroundShadowRenderer2D implements AutoCloseable {
    ...
    public GroundShadowRenderer2D() {
        try(var state=new RenderStateSnapshot3D()) {
            ...
            glEnableVertexAttribArray(0);glVertexAttribPointer(0,3,GL_FLOAT,false,20,0);
            glEnableVertexAttribArray(1);glVertexAttribPointer(1,2,GL_FLOAT,false,20,12);
        }
    }
    ...
    public void draw(TextureBatch sceneBatch,PointLight2D light,List<SpriteGroundShadow2D> casters,PointLight2D[] fills) {
        ...
        drawCalls=0;
        if(!light.enabled || !light.shadows || light.elevation<=0 || light.intensity<=0 || light.r+light.g+light.b<=0)return;
        sceneBatch.flush();
        try(var state=new RenderStateSnapshot3D()) {
            glDisable(GL_DEPTH_TEST);glDepthMask(false);glDisable(GL_CULL_FACE);
            glEnable(GL_BLEND);glBlendEquation(GL_FUNC_ADD);
            glBlendFuncSeparate(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA,GL_ZERO,GL_ONE);
            glActiveTexture(GL_TEXTURE2);glBindSampler(2,0);
            shader.bind();shader.setUniform1i("u_texture",2);
            ...
            for(int n=0;n<casters.size();n++) {
                ...
                if(!caster.enabled || caster.height<=0 || caster.opacity<=0 || caster.sprite.getColor().a()<=0)continue;
                var quad=caster.sprite.getVertexBuffer();var uv=caster.sprite.getUVBuffer();
                ...
                if(fills!=null){
                    ...
                    for(PointLight2D fill:fills){float weight=weight(fill,baseX,baseY);sum+=weight;sx+=fill.x*weight;sy+=fill.y*weight;sz+=fill.elevation*weight;}
                    if(sum>0){blended.setPosition(sx/sum,sy/sum).setElevation(Math.max(80,sz/sum));source=blended;}
                }
                ...
                float height=SpriteGroundShadow2D.boundedHeight(caster.height,source.elevation,
                        distance+Math.abs(dx)+Math.abs(vx),caster.maxLength);
                vertices.clear();
                ...
                vertices.flip();glBufferSubData(GL_ARRAY_BUFFER,0,vertices);
                ...
                shader.setUniform4f("u_uvBounds",Math.min(uv.get(0),uv.get(4))+halfU,Math.min(uv.get(1),uv.get(5))+halfV,
                        Math.max(uv.get(0),uv.get(4))-halfU,Math.max(uv.get(1),uv.get(5))-halfV);
                shader.setUniform1f("u_opacity",caster.opacity*caster.sprite.getColor().a());
                shader.setUniform1i("u_contact",0);
                glDrawArrays(GL_TRIANGLE_STRIP,0,4);drawCalls++;
                ...
                vertices.clear();
                for(int c=0;c<4;c++){float u=c&1,v=(c>>1);vertices.put(baseX+(u*2-1)*radius).put(baseY+(v*2-1)*radius*.4f).put(1).put(u).put(v);}
                vertices.flip();glBufferSubData(GL_ARRAY_BUFFER,0,vertices);
                shader.setUniform1i("u_contact",1);glDrawArrays(GL_TRIANGLE_STRIP,0,4);drawCalls++;
            }
        }
    }
    ...
    @Override public void close(){if(closed)return;closed=true;shader.dispose();glDeleteBuffers(vbo);glDeleteVertexArrays(vao);}
}
