...
/**
 * NanoVG variant. Horizontal ancestor navigation from a configured root to a current path. Buttons
 ...
 * <pre>{@code
 * NanoBreadcrumbBar crumbs = new NanoBreadcrumbBar().path(projectRoot, currentDirectory);
 ...
 * Place it in a horizontal NanoScrollPanel when arbitrarily long paths must fit.
 * @author Albert Beaupre
 */
public class NanoBreadcrumbBar extends NanoContainer {
     ...
     */
    public NanoBreadcrumbBar() { getLayout().row().height(32).noShrink(); }
     ...
     */
    public NanoBreadcrumbBar path(Path root, Path current) {
    ...
    private void append(Path ancestor) {
        ...
        NanoButton button = new NanoButton(name).action(b -> { if (isEnabled()) navigate.accept(ancestor); });
     ...
     * @return this bar
     */
    public NanoBreadcrumbBar onNavigate(Consumer<Path> listener) { navigate = Objects.requireNonNull(listener); return this; }
}
