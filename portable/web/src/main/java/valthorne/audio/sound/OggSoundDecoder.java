...
public class OggSoundDecoder implements SoundDecoder {
 ...
 public SoundData decode(byte[] bytes){validate(bytes);SoundData data=valthorne.web.audio.AudioDecode.decode(null,bytes,AudioFormat.OGG);return new SoundData(null,data.data(),0,data.data().remaining(),data.duration(),data.channels(),data.sampleRate(),data.bitsPerSample(),false,true,AudioFormat.OGG);}
}
