...
public final class CommonSceneApplication implements Application {
    ...
    public static void main(String[] args){try{JGL.init(new CommonSceneApplication(),JGLConfiguration.defaults().title("Shared 3D scene / B light / N camera / Escape close").contextVersion(4,1).size(960,640).visible(false));System.out.println("COMMON_SCENE_VALIDATED");}catch(Throwable failure){failure.printStackTrace();throw failure;}}
    public void init(){
        sceneRenderer=new SceneRenderer3D();renderer=sceneRenderer.filament();renderer.setQuality(FilamentRenderer3D.Quality.PERFORMANCE);renderer.setAntiAliasing(true);renderer.setEnvironmentIntensity(500);
        world=new PhysicsWorld3D(new PhysicsWorld3D.Settings(1f/60,8,64,2048,2048,0));
        camera.setPosition(7,-9,6);camera.lookAt(0,0,1.5f,0,0,1);camera.setFieldOfViewDegrees(50);camera.setClipPlanes(.1f,100);
        ByteBuffer pixels=ByteBuffer.allocateDirect(16);pixels.put(new byte[]{(byte)255,30,30,(byte)255,30,(byte)255,30,(byte)255,30,30,(byte)255,(byte)255,(byte)160,(byte)160,(byte)160,(byte)255}).flip();
        texture=new Texture(new TextureData(pixels,2,2));overlay=new TextureBatch(8);
        ...
        for(int i=0;i<6;i++){
            var instance=new ModelInstance3D().setModel(box).setMaterial(new Material3D().setTint(new Color(.12f+i*.12f,.35f,.8f-i*.1f,1)).setRoughness(.2f+i*.1f));scene.add(instance);
        ...
        parent.addChild(new SceneNode3D().setModel(box).setPosition(0,0,1).setScale(.5f,.5f,2));scene.addNode(parent);
        scene.add(new ModelInstance3D().setModel(box).setPosition(500,0,0).setMaterial(new Material3D().setCastsShadow(false)));
        key=new PointLight3D().setPosition(-3,-4,7).setColor(new Color(1,.85f,.65f,1)).setIntensity(500).setRange(35).setCastsShadows(true);scene.addLight(key);
    ...
    public void update(float dt){
        ...
        if(Keyboard.isKeyDown(Keyboard.N)){camera.setPosition(-9,-13,8);camera.lookAt(0,0,1.5f,0,0,1);}
        if(Keyboard.isKeyDown(Keyboard.ESCAPE)||frames>=600)Window.requestClose();
    }
    public void render(){
        sceneRenderer.render(scene,camera);frames++;
        overlay.begin();overlay.draw(texture,20,20,80,80);overlay.end();
        if(frames==2){if(renderer.getCachedMeshCount()!=2||renderer.getOffscreenCount()<1||renderer.getPointLightCount()<3)throw new AssertionError("Shared renderer state differs");System.out.println("COMMON_SCENE_READY");}
        if(frames==100){renderer.invalidate();System.out.println("COMMON_SCENE_STATS vertices="+renderer.getUploadedSourceVertices()+" unique="+renderer.getUploadedUniqueVertices());}
    }
    public void dispose(){if(world!=null)world.close();if(renderer!=null){scene.clear();renderer.render(scene,camera);if(renderer.getCachedMeshCount()!=0||renderer.getPointLightCount()!=0)throw new AssertionError("Scene retained resources");sceneRenderer.close();sceneRenderer.close();}if(overlay!=null)overlay.dispose();if(texture!=null)texture.dispose();}
}
