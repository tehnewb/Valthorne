...
public final class OcclusionCuller3D {
     ...
     */
    public int getOccluderCount() {return count;}
    ...
    public void addOccluder(Model3D model, Matrix4fc world, Material3D material) {
        ...
        if (model == null || material == null || count == LIMIT || scanned >= SCAN_LIMIT
                ...
                || material.isCullBackFaces() || !(material.getTint().a() >= 1) || material.getAlphaCutoff() >= 1) return;
        ...
        for (var triangle : model.triangles()) {
            ...
            triangles[at] = points[0]; triangles[at + 1] = points[1];
            triangles[at + 2] = points[b]; triangles[at + 3] = points[b + 1];
            triangles[at + 4] = points[c]; triangles[at + 5] = points[c + 1];
    ...
    private Visibility testBounds(AABBf b) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (!(Float.isFinite(px) && Float.isFinite(py) && Float.isFinite(pz) && Float.isFinite(w))) return Visibility.VISIBLE;
            commonOutside &= (px < -(1 + marginX)*w ? 1 : 0) | (px > (1 + marginX)*w ? 2 : 0)
                    | (py < -(1 + marginY)*w ? 4 : 0) | (py > (1 + marginY)*w ? 8 : 0)
                    | (pz < -w - 1e-4f ? 16 : 0) | (pz > w + 1e-4f ? 32 : 0);
            if (w <= 1e-5f || pz <= -w + 1e-5f) {safeProjection = false; continue;}
            px /= w; py /= w; pz /= w;
            minX = Math.min(minX, px); maxX = Math.max(maxX, px);
            minY = Math.min(minY, py); maxY = Math.max(maxY, py); minZ = Math.min(minZ, pz);
        }
    ...
    private boolean project(Vector3f v, int at) {
        ...
        points[at] = x/w; points[at+1] = y/w; points[at+2] = z/w;
     ...
     */
    private void requireActive() {if (!active) throw new IllegalStateException("begin must precede visibility tests");}
}
