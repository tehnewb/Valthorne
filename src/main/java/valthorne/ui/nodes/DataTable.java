...
public class DataTable<T> extends Panel {
    ...
    private final valthorne.ui.nodes.nano.NanoLabel empty = new valthorne.ui.nodes.nano.NanoLabel("No matching rows"); // Owned empty-projection message.
    ...
    public DataTable(List<TableColumn<T>> columns) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            int index = i; TableColumn<T> column = columns.get(i);
        ...
        model.onChange(() -> {
            ...
            rows.itemCount(model.size()); rows.scrollY(0);
     ...
     */
    public Button getHeader(int index) { return buttons[Objects.checkIndex(index, buttons.length)]; }
    /**
     ...
     */
    public DataTable<T> rows(List<? extends T> data) { model.rows(data); return this; }
    /**
     ...
     */
    public DataTable<T> filter(Predicate<? super T> predicate) { model.filter(predicate); return this; }
    /**
     ...
     */
    public DataTable<T> rowHeight(float height) { rows.rowHeight(height); return this; }
    /**
     ...
     */
    public boolean isDescending() { int column = getSortedColumn(); return column >= 0 && model.comparator() == reverse.get(column); }
    /**
    ...
    public void toggleSort(int index) {
        ...
        int sortedColumn = getSortedColumn(); boolean descending = isDescending();
    ...
    private UINode createRow(int index) {
        ...
        Panel row = new Panel(); row.setStyleName("table-row"); row.setFocusable(true);
        row.getLayout().row().itemsCenter();
        for (TableColumn<T> column : columns) {
            Panel cell = new Cell(); cell.setClickable(false);
    ...
    private static final class Cell extends Panel {
        ...
        @Override public void draw(valthorne.graphics.texture.TextureBatch batch) {
            ...
            try { super.draw(batch); } finally { batch.endScissor(); }
        }
    }
}
