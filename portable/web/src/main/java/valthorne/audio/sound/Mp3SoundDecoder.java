...
public class Mp3SoundDecoder implements SoundDecoder {
 ...
 public SoundData decode(byte[] bytes){SoundData data=AudioFormat.detect(bytes)==AudioFormat.WAV?new WaveSoundDecoder().decode(bytes):valthorne.web.audio.AudioDecode.decode(null,bytes,AudioFormat.MP3);return new SoundData(null,data.data(),0,data.data().remaining(),data.duration(),data.channels(),data.sampleRate(),data.bitsPerSample(),false,true,AudioFormat.MP3);}
}
