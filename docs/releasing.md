...
The stable version is `2.3.0` in `gradle.properties`. Normal builds and verification
...
4. Inspect `build/docs/javadoc/` and
   `build/release-repository/io/github/tehnewb/Valthorne/2.3.0/`. Library, sources
...
After publication, resolve `io.github.tehnewb:Valthorne:2.3.0` from Maven Central
in an independent consumer, verify the artifact/signature files, and tag the
validated commit `v2.3.0`. Include the JARs, sources, Javadoc and checksums in the
