...
public final class SpriteVolumeRenderer2D implements AutoCloseable {
    ...
    public SpriteVolumeRenderer2D(){
        try(var state=new RenderStateSnapshot3D()){
            ...
            vao=glGenVertexArrays();vbo=glGenBuffers();glBindVertexArray(vao);glBindBuffer(GL_ARRAY_BUFFER,vbo);glBufferData(GL_ARRAY_BUFFER,96,GL_STREAM_DRAW);
            for(int i=0;i<3;i++){glEnableVertexAttribArray(i);glVertexAttribPointer(i,2,GL_FLOAT,false,24,i*8L);}
        }
    }
    public void draw(TextureBatch batch,SpriteGroundShadow2D card,PointLight2D overhead,PointLight2D[] fills,float opacity){
        ...
        PointLight2D.nonnegative(opacity);if(opacity>1)throw new IllegalArgumentException("Opacity exceeds one");
        var sprite=card.sprite;var quad=sprite.getVertexBuffer();var uv=sprite.getUVBuffer();
        ...
        batch.flush();
        try(var state=new RenderStateSnapshot3D()){
            glDisable(GL_DEPTH_TEST);glDepthMask(false);glDisable(GL_CULL_FACE);glEnable(GL_BLEND);glBlendEquation(GL_FUNC_ADD);
            glBlendFuncSeparate(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA,GL_ONE,GL_ONE_MINUS_SRC_ALPHA);
            glActiveTexture(GL_TEXTURE2);glBindSampler(2,0);glBindTexture(GL_TEXTURE_2D,sprite.getTexture().getTextureID());
            shader.bind();shader.setUniform1i("u_texture",2);shader.setUniformMatrix4("u_projection",Window.getProjectionMatrix());
            ...
            setLight(0,overhead);int count=1;
            if(fills!=null)for(PointLight2D light:fills){if(count==5)break;setLight(count++,light);}
            shader.setUniform1i("u_count",count);
            vertices.clear();
            // Sprite vertex buffers are perimeter ordered; triangle strips need 0,1,3,2.
            for(int c=0;c<4;c++){int index=c<2?c:5-c;vertices.put(quad.get(index*2)).put(quad.get(index*2+1));
                vertices.put(uv.get(index*2)).put(uv.get(index*2+1));vertices.put(c&1).put(c>>1);}
            vertices.flip();glBindVertexArray(vao);glBindBuffer(GL_ARRAY_BUFFER,vbo);glBufferSubData(GL_ARRAY_BUFFER,0,vertices);glDrawArrays(GL_TRIANGLE_STRIP,0,4);
        }
    }
    ...
    public void close(){if(closed)return;closed=true;shader.dispose();glDeleteBuffers(vbo);glDeleteVertexArrays(vao);}
}
