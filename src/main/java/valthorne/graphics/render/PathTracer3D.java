...
public final class PathTracer3D implements AutoCloseable {
    ...
    private final int denoise;
    private final int vao;
    private int accumulation;
    private int atlas;
    private int guide; // Owned program, vertex-array, radiance, texture-array, and geometry-guide names.
    ...
    public PathTracer3D() {
        if (!GL.getCapabilities().OpenGL43)
            throw new IllegalStateException("PathTracer3D requires an OpenGL 4.3 context; request contextVersion(4,3), or use Lighting3D.");
        try {
     ...
     */
    private static int floatTexture(int w, int h) {
    ...
    private static int program(int[] types, String[] sources) {
        ...
        try {
            for (int i = 0; i < types.length; i++) {
                int s = glCreateShader(types[i]);
                shaders.add(s);
                glShaderSource(s, sources[i]);
                glCompileShader(s);
    ...
    public PathTracer3D setExposure(float value) {
        if (!Float.isFinite(value) || value <= 0) throw new IllegalArgumentException("Exposure must be positive");
        exposure = value;
        return this;
    }
    ...
    public PathTracer3D setSky(float r, float g, float b) {
        if (!Float.isFinite(r + g + b) || Math.min(r, Math.min(g, b)) < 0)
            throw new IllegalArgumentException("Sky must be finite and nonnegative");
        if (r != skyR || g != skyG || b != skyB) samples = 0;
        if (r != skyR || g != skyG || b != skyB) historyValid = false;
        skyR = r;
        skyG = g;
        skyB = b;
        return this;
    ...
    public PathTracer3D setMaxSamples(int value) {
        if (value < 1 || value > 1000000) throw new IllegalArgumentException("Samples must be in [1,1000000]");
        maxSamples = value;
        return this;
    }
    ...
    public void render(Scene3D scene, Camera3D camera) {
        if (Thread.currentThread() != owner)
            throw new IllegalStateException("Use path tracer on its creating GL thread");
        if (closed) throw new IllegalStateException("Path tracer is closed");
        ...
        try (State saved = new State()) {
            ...
            float[] matrix = camera.getCombined().get(matrixUpload);
            ...
            if (invalid || signature != snapshot.signature()) {
                try {
                    ...
                    glBufferData(GL_SHADER_STORAGE_BUFFER, snapshot.triangleData(), GL_STATIC_DRAW);
                    glBindBuffer(GL_SHADER_STORAGE_BUFFER, buffers[1]);
                    glBufferData(GL_SHADER_STORAGE_BUFFER, snapshot.nodeData(), GL_STATIC_DRAW);
                    glBindBuffer(GL_SHADER_STORAGE_BUFFER, buffers[2]);
                    glBufferData(GL_SHADER_STORAGE_BUFFER, snapshot.emitterData(), GL_STATIC_DRAW);
                    uploadAtlas(snapshot.textures);
                    triangleCount = snapshot.triangles.size();
                    nodeCount = snapshot.nodes.size();
                    emitterCount = snapshot.emitters.size();
            ...
            if (w != width || h != height) {
                if (accumulation != 0) glDeleteTextures(accumulation);
                accumulation = glGenTextures();
                ...
                samples = 0;
                historyValid = false;
                glDeleteTextures(guide);
                guide = floatTexture(w, h);
                for (int i = 0; i < 2; i++) {
                    glDeleteTextures(filtered[i]);
                    filtered[i] = floatTexture(w, h);
                }
                glDeleteTextures(specularRadiance);
                specularRadiance = floatTexture(w, h);
                glDeleteTextures(materialGuide);
                materialGuide = floatTexture(w, h);
                for (int i = 0; i < 2; i++) {
                    glDeleteTextures(filteredSpecular[i]);
                    filteredSpecular[i] = floatTexture(w, h);
                }
                glDeleteTextures(historySpecular);
                historySpecular = 0;
                glDeleteTextures(historyRadiance);
                historyRadiance = 0;
                glDeleteTextures(historyGuide);
                historyGuide = 0;
            }
            if (live && historyRadiance == 0) {
                historyRadiance = floatTexture(w, h);
                historyGuide = floatTexture(w, h);
                historySpecular = floatTexture(w, h);
            }
            if (samples < maxSamples) {
                if (live && historyValid) {
                    int swap = accumulation;
                    accumulation = historyRadiance;
                    historyRadiance = swap;
                    swap = guide;
                    guide = historyGuide;
                    historyGuide = swap;
                }
                if (live && historyValid) {
                    int swap = specularRadiance;
                    specularRadiance = historySpecular;
                    historySpecular = swap;
                }
                glUseProgram(compute);
                for (int i = 0; i < 3; i++) glBindBufferBase(GL_SHADER_STORAGE_BUFFER, i, buffers[i]);
                glBindImageTexture(0, accumulation, 0, false, 0, GL_READ_WRITE, GL_RGBA32F);
                glActiveTexture(GL_TEXTURE0);
                glBindTexture(GL_TEXTURE_2D_ARRAY, atlas);
                glBindSampler(0, 0);
                glBindImageTexture(1, guide, 0, false, 0, GL_WRITE_ONLY, GL_RGBA32F);
                glBindImageTexture(2, specularRadiance, 0, false, 0, GL_READ_WRITE, GL_RGBA32F);
                glBindImageTexture(6, materialGuide, 0, false, 0, GL_READ_WRITE, GL_RGBA32F);
                uniform("atlas", 0);
                uniform("triangleCount", triangleCount);
                uniform("emitterCount", emitterCount);
                uniform("sampleOffset", samples);
                int frameSequence = sequence++;
                float jitterX = live ? radicalInverse((frameSequence & 1023) + 1, 2) : .5f, jitterY = live ? radicalInverse((frameSequence & 1023) + 1, 3) : .5f;
                glUniform2f(glGetUniformLocation(compute, "pixelJitter"), jitterX, jitterY);
                glUniform2f(glGetUniformLocation(compute, "previousJitter"), previousJitterX, previousJitterY);
                uniform("realtime", live ? 1 : 0);
                uniform("historyValid", live && historyValid ? 1 : 0);
                uniform("sequence", frameSequence);
                uniform("cameraMoving", cameraMoving ? 1 : 0);
                uniform("maxHistory", maxSamples);
                if (live) {
                    glBindImageTexture(3, historyRadiance, 0, false, 0, GL_READ_ONLY, GL_RGBA32F);
                    glBindImageTexture(4, historyGuide, 0, false, 0, GL_READ_ONLY, GL_RGBA32F);
                    glBindImageTexture(5, historySpecular, 0, false, 0, GL_READ_ONLY, GL_RGBA32F);
                    if (historyValid) {
                        glUniformMatrix4fv(glGetUniformLocation(compute, "previousCamera"), false, previousMatrix);
                        glUniformMatrix4fv(glGetUniformLocation(compute, "previousInverse"), false, previousInverse);
                        glUniform3fv(glGetUniformLocation(compute, "previousPosition"), previousPosition);
                        uniform("previousPerspective", previousPerspective ? 1 : 0);
                    }
                }
                int count = Math.min(quality.samples, maxSamples - samples);
                uniform("samplesPerFrame", count);
                uniform("maxBounces", quality.bounces);
                uniform("perspective", camera instanceof PerspectiveCamera ? 1 : 0);
                ...
                glUniform3f(glGetUniformLocation(compute, "cameraPosition"), p.x(), p.y(), p.z());
                ...
                filterDirty = true;
                if (live) {
                    previousMatrix = matrix.clone();
                    previousInverse = camera.getInverseCombined().get(inverseUpload).clone();
                    previousPosition[0] = p.x();
                    previousPosition[1] = p.y();
                    previousPosition[2] = p.z();
                    previousPerspective = camera instanceof PerspectiveCamera;
                    historyValid = true;
                }
            ...
            if (denoising && filterDirty) {
                glUseProgram(denoise);
                glBindImageTexture(1, guide, 0, false, 0, GL_READ_ONLY, GL_RGBA32F);
                ...
                for (int pass = 0; pass < 3; pass++) {
                    glBindImageTexture(0, output, 0, false, 0, GL_READ_ONLY, GL_RGBA32F);
                    output = filtered[pass % 2];
                    glBindImageTexture(2, output, 0, false, 0, GL_WRITE_ONLY, GL_RGBA32F);
                    glBindImageTexture(4, specularOutput, 0, false, 0, GL_READ_ONLY, GL_RGBA32F);
                    specularOutput = filteredSpecular[pass % 2];
                    glBindImageTexture(5, specularOutput, 0, false, 0, GL_WRITE_ONLY, GL_RGBA32F);
                    glUniform1i(glGetUniformLocation(denoise, "stepWidth"), 1 << pass);
                    glDispatchCompute((width + 7) / 8, (height + 7) / 8, 1);
                    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT);
                }
                filterDirty = false;
            ...
            if (denoising) {
                output = filtered[0];
                specularOutput = filteredSpecular[0];
            }
            ...
            glBindSampler(0, 0);
            glUniform1i(glGetUniformLocation(present, "resultImage"), 0);
            glUniform1f(glGetUniformLocation(present, "exposure"), exposure);
            glActiveTexture(GL_TEXTURE1);
            glBindTexture(GL_TEXTURE_2D, specularOutput);
            glBindSampler(1, 0);
            glUniform1i(glGetUniformLocation(present, "specularImage"), 1);
            glActiveTexture(GL_TEXTURE2);
            glBindTexture(GL_TEXTURE_2D, materialGuide);
            glBindSampler(2, 0);
            glUniform1i(glGetUniformLocation(present, "materialImage"), 2);
     ...
     */
    private void uniform(String name, int value) {
        glUniform1i(glGetUniformLocation(compute, name), value);
    }
    ...
    private void uploadAtlas(List<Texture> textures) {
        if (textures.size() > glGetInteger(GL_MAX_ARRAY_TEXTURE_LAYERS))
            throw new IllegalArgumentException("Too many path tracing textures");
        if (atlas != 0) glDeleteTextures(atlas);
        atlas = glGenTextures();
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D_ARRAY, atlas);
        final int side = 512;
        glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_RGBA8, side, side, Math.max(1, textures.size()));
        ...
        for (int layer = 0; layer < textures.size(); layer++) {
            glBindTexture(GL_TEXTURE_2D, textures.get(layer).getTextureID());
            ...
            try {
                ...
                for (int y = 0; y < side; y++)
                    for (int x = 0; x < side; x++) {
                        float sx = (x + .5f) * w / side - .5f, sy = (y + .5f) * h / side - .5f;
                        int x0 = (int) Math.floor(sx), y0 = (int) Math.floor(sy);
                        float fx = sx - x0, fy = sy - y0;
                        for (int c = 0; c < 4; c++) {
                            float a = source.get((Math.floorMod(y0, h) * w + Math.floorMod(x0, w)) * 4 + c) & 255, b = source.get((Math.floorMod(y0, h) * w + Math.floorMod(x0 + 1, w)) * 4 + c) & 255;
                ...
                target.flip();
                glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, layer, side, side, 1, GL_RGBA, GL_UNSIGNED_BYTE, target);
            } finally {
    ...
    public void close() {
        ...
        glDeleteProgram(denoise);
        glDeleteVertexArrays(vao);
        glDeleteTextures(specularRadiance);
        glDeleteTextures(historySpecular);
        glDeleteTextures(materialGuide);
        glDeleteTextures(filteredSpecular);
        glDeleteTextures(accumulation);
        glDeleteTextures(historyRadiance);
        glDeleteTextures(historyGuide);
        glDeleteTextures(atlas);
        glDeleteTextures(guide);
        glDeleteTextures(filtered);
        for (int b : buffers) glDeleteBuffers(b);
    }
    ...
    public enum Quality {
        ...
        ULTRA(1, 4, 12);
        final float scale;
        final int samples, bounces; // Progressive resolution multiplier, nominal frame samples, and maximum path depth.
         ...
         */
        Quality(float scale, int samples, int bounces) {
            this.scale = scale;
    ...
    private static final class State implements AutoCloseable {
        ...
        final int[] pixelValues = new int[pixelNames.length], indexed = new int[3];
        final long[] starts = new long[3], sizes = new long[3]; // Captured pixel settings and indexed shader-storage buffer names, offsets, and sizes.
        final int[] imageNames = {GL_IMAGE_BINDING_NAME, GL_IMAGE_BINDING_LEVEL, GL_IMAGE_BINDING_LAYERED, GL_IMAGE_BINDING_LAYER, GL_IMAGE_BINDING_ACCESS, GL_IMAGE_BINDING_FORMAT};
        final int[][] images = new int[7][6]; // Image binding property enums and saved values for image units zero through six.
        final int[] otherTextures = new int[2], otherSamplers = new int[2]; // Captured 2D texture and sampler bindings for units one and two.
        final int texture, array, sampler;
        final boolean depth = glIsEnabled(GL_DEPTH_TEST), blend = glIsEnabled(GL_BLEND), cull = glIsEnabled(GL_CULL_FACE), srgb = glIsEnabled(GL_FRAMEBUFFER_SRGB); // Captured unit-zero texture/sampler bindings and rendering capability flags.
        ...
        State() {
            for (int i = 0; i < 2; i++) {
                ...
                otherTextures[i] = glGetInteger(GL_TEXTURE_BINDING_2D);
                otherSamplers[i] = glGetIntegeri(GL_SAMPLER_BINDING, i + 1);
            }
            glActiveTexture(GL_TEXTURE0);
            texture = glGetInteger(GL_TEXTURE_BINDING_2D);
            array = glGetInteger(GL_TEXTURE_BINDING_2D_ARRAY);
            sampler = glGetIntegeri(GL_SAMPLER_BINDING, 0);
            for (int i = 0; i < 7; i++) for (int j = 0; j < 6; j++) images[i][j] = glGetIntegeri(imageNames[j], i);
            for (int i = 0; i < 3; i++) {
                indexed[i] = glGetIntegeri(GL_SHADER_STORAGE_BUFFER_BINDING, i);
                starts[i] = glGetInteger64i(GL_SHADER_STORAGE_BUFFER_START, i);
                sizes[i] = glGetInteger64i(GL_SHADER_STORAGE_BUFFER_SIZE, i);
            }
            glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
            glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
            for (int i = 0; i < pixelNames.length; i++) {
                pixelValues[i] = glGetInteger(pixelNames[i]);
                glPixelStorei(pixelNames[i], i == 0 || i == 4 ? 1 : 0);
            }
        }
         ...
         */
        private static void enable(int flag, boolean value) {
        ...
        public void close() {
            for (int i = 0; i < 3; i++) {
                if (indexed[i] != 0 && sizes[i] > 0)
                    glBindBufferRange(GL_SHADER_STORAGE_BUFFER, i, indexed[i], starts[i], sizes[i]);
                else glBindBufferBase(GL_SHADER_STORAGE_BUFFER, i, indexed[i]);
            }
            glBindBuffer(GL_SHADER_STORAGE_BUFFER, buffer);
            for (int i = 0; i < 7; i++) {
                int[] im = images[i];
                glBindImageTexture(i, im[0], im[1], im[2] != 0, im[3], im[4], im[5]);
            }
            for (int i = 0; i < pixelNames.length; i++) glPixelStorei(pixelNames[i], pixelValues[i]);
            ...
            for (int i = 0; i < 2; i++) {
                ...
                glBindSampler(i + 1, otherSamplers[i]);
            }
