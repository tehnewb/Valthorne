...
public final class Material3D {
    ...
    public Material3D copy() {
        return new Material3D()
                ...
                .setLightingMix(lightingMix)
                .setFogMix(fogMix)
                .setRadianceMix(radianceMix)
                .setRoughness(roughness).setMetallic(metallic)
                .setTransmission(transmission).setIndexOfRefraction(indexOfRefraction).setEmissionStrength(emissionStrength)
                .setEmissionLightEnabled(emissionLightEnabled)
                .setDepthTest(depthTest)
                .setDepthWrite(depthWrite)
                .setCullBackFaces(cullBackFaces)
                .setTexture(texture)
                .setAlphaCutoff(alphaCutoff)
                .setCastsShadow(castsShadow).setReceivesShadow(receivesShadow);
    }
}
